...
export const validateTheme = (theme: Theme): boolean => {
  const requiredColorProperties = [
    'background.primary', 'background.secondary', 'background.sidebar', 'background.chat',
    'text.primary', 'text.secondary', 'text.muted', 'text.inverse',
    'border.primary', 'border.secondary', 'border.focus',
    'primary.DEFAULT', 'primary.hover', 'primary.active',
    'chat.userBubble', 'chat.assistantBubble', 'chat.userText', 'chat.assistantText',
  ...
  for (const prop of requiredColorProperties) {
    ...
    for (const key of path) {
      if (!current || typeof current[key] === 'undefined') {
        console.warn(`Missing theme property: colors.${prop} in theme ${theme.id}`);
        return false;
      }
      current = current[key];
    }
